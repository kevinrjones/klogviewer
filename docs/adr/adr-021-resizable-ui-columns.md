...
1.  **State Management**: Added a `columnWidths: Map<String, Int>` property to `LogWindow` and persisted it in `UserPreferences`.
2.  **Interaction Pattern**: Users can drag the right edge of any column header to resize it.
3.  **Visual Feedback**: The mouse cursor changes to a horizontal resize icon (`E_RESIZE_CURSOR`) when hovering over the draggable areas between columns.
